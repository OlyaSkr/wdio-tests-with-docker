...
describe('Footer section', () => {
  ...
  it('Verify item titles in Platform section ', async () => {
    allureReporter.addDescription(
      'Verifies item titles in the Platform section of the footer'
    );

    allureReporter.startStep('Get ai agent platform items titles');
    const platformItemsTitles = await footerPage.getTitlesInFooterItems(
      footerData.expectedFooterSectionTitles[0]
    );
    allureReporter.endStep();

    allureReporter.startStep('Compare with expected Platform items titles');
    const expectedPlatform = footerData.platformItemsTitles;
    expectedPlatform.forEach((title) => {
      expect(platformItemsTitles).toContain(title);
    });
    allureReporter.endStep();
  });
  ...
  it('Verify item titles in Company section ', async () => {
    ...
    const companyItemsTitles = await footerPage.getTitlesInFooterItems(
      footerData.expectedFooterSectionTitles[5]
    );
    allureReporter.endStep();
  ...
  it('Should open each link in Platform section', async () => {
    allureReporter.addDescription(
      'Verifies that each link in the Platform section opens the expected page'
    );

    const aiAgentPlatformsection = footerData.expectedFooterSectionTitles[0];
    const expectedTitles = footerData.platformLinkTitles;
    const expectedUrls = footerData.platformLinkEndpoints;
  ...
  it('Should open each link in Resources section', async () => {
    ...
    for (let i = 0; i < expectedTitles.length; i++) {
      ...
      await browser.waitUntil(
        async () =>
          !(
            await footerPage.getCurrentPageTitle()
          ).includes('Just a moment...'),
        {
          timeout: 40000,
          timeoutMsg: `"Just a moment..." still displayed for link #${i + 1}`,
        }
      );
  ...
  it('Should open each link in Company section', async () => {
    ...
    const companySection = footerData.expectedFooterSectionTitles[5];
  ...
  it('Verify footer nav links in About page', async () => {
    ...
    for (let i = 0; i < expectedTitles.length; i++) {
      ...
      await browser.waitUntil(
        ...
        {
          timeout: 40000,
          timeoutMsg: `"Just a moment..." still displayed for title #${i + 1}`,
        }
      );
      await footerPage.waitForTitle(expectedTitles[i]);
